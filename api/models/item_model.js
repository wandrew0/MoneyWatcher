...
item_schema.methods.sync = async function () {
    ...
    try {
        await mongoose.connection.transaction(async (session) => {
            ...
            const institution_name = await plaid.get_institution_name(
                institution_id_accounts["item"]["institution_id"]
            );
            ...
            for (const transaction of transactions) {
                ...
                // do merchant
                let exists = await Merchant.exists({
                    user_uuid: this.user_uuid,
                    name: transaction["name"]
                }).session(session);
            ...
            // https://stackoverflow.com/questions/70756890/mongoose-doesnt-abort-the-transaction-when-promise-all-rejects ?
    ...
    for (const name of Object.keys(merchants)) {
        Merchant.create([merchants[name]]).catch((err) =>
