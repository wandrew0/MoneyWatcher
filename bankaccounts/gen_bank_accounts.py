...
def generate_transaction_list(date: datetime.date, count: int) -> List[Dict[str, any]]:
    ret = []
    for _ in range(count):
        transaction = {
            "date_transacted": date.strftime("%Y-%m-%d"),
...
def main():
    ...
    with open(os.path.dirname(__file__) + "/" + config.get("out"), "w") as outfile:
